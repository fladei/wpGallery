$(function(){
	...
	$("#s").keyup( function(){
        ...
        t = setTimeout(sajax_do_call( 'retrieveResultsWhileTyping', params, populateOptions ), 1500); // Only do the ajax call when the user has stoped typing for more than 1.5 seconds so the server isn't overwhelmed
	});
	
function populateOptions (result)
{	
	var datafromServer = result.responseText.split(",");
    $("#s").autocomplete({
        source: datafromServer
    })
}

$('#gallerySearchForm').submit(function() {
	var params = new Array();
	params[0] = $('#s').val();
	...
	sajax_do_call( 'searchImageGallery', params, listImages );
	return false;
});

function listImages ( result )
{	
	// If no results were found, show error and exit
	if (result.responseText.length == 0 || result.responseText === "" || result == null)
	{
		$.unblockUI();
		$.blockUI({ 
			css: { 
			...
			message: '<h1>No results found</h1>'
		});
	...
	// Grab results
	var results = result.responseText.split( '-EOI-' );
	
	// Generate HTML 
	$('#bodyContent').append('<ul id="myGallery">');

	for ( var i = 0 ; i < results.length ; i++ )
	{
		//alert(results[ i ]); // uncomment for debugging
		$('#myGallery').append(results[ i ]);
	}
	...
	// Avoid sending the page - ajax effect
	$.unblockUI();
	return false;
}